...
public class JwtOrgResolver implements TenantResolver {
    ...
    public String resolveTenantId() {
        try {
            if (!Arc.container().requestContext().isActive()) { // can happen during testing
                return fallbackToDefault("request context not active");
            }

            HttpServerRequest request = requestInstance.get();
            if (request == null) { // can happen during testing
            ...
            // HttpServerRequest producer returns null during startup (Flyway, Bootstrap services using on `@Observes StartupEvent`)
    ...
    private String fallbackToDefault(String reason) {
        ...
        log.warn("Falling back to DEFAULT_ORG_ID (" + reason + ")");
        return DEFAULT_ORG_ID;
    }
